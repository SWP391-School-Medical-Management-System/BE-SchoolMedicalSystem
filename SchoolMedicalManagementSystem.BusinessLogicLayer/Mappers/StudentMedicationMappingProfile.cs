...
namespace SchoolMedicalManagementSystem.BusinessLogicLayer.Mappers
{
    public class StudentMedicationMappingProfile : Profile
    {
        public StudentMedicationMappingProfile()
        {
            ...
            ConfigureUsageHistoryMappings(); // Add new method for usage history mappings
        }
        ...
        private void ConfigureResponseMappings()
        {
            ...
            CreateMap<StudentMedication, StudentMedicationResponseForRequest>()
                ...
                .ForMember(dest => dest.QuantityUnit, opt => opt.MapFrom(src => src.QuantityUnit))
                .ForMember(dest => dest.QuantityReceived, opt => opt.MapFrom(src => src.QuantityReceive))
            ...
            CreateMap<StudentMedicationUsageHistory, StudentMedicationUsageHistoryResponse>()
                ...
                .ForMember(dest => dest.DosageUse, opt => opt.MapFrom(src => src.DosageUse))
                .ForMember(dest => dest.Status, opt => opt.MapFrom(src => src.Status))
                .ForMember(dest => dest.StatusDisplayName, opt => opt.MapFrom(src => GetStatusUsageDisplayName(src.Status)))
                .ForMember(dest => dest.Reason, opt => opt.MapFrom(src => src.Reason))
                .ForMember(dest => dest.Note, opt => opt.MapFrom(src => src.Note))
                .ForMember(dest => dest.AdministeredBy, opt => opt.MapFrom(src => src.AdministeredBy))
                .ForMember(dest => dest.AdministeredByName, opt => opt.MapFrom(src => src.Nurse != null ? src.Nurse.FullName : ""))
        ...
        private void ConfigureUsageHistoryMappings()
        {
            CreateMap<StudentMedicationUsageHistory, StudentMedicationUsageHistoryResponse>()
                .ForMember(dest => dest.Id, opt => opt.MapFrom(src => src.Id))
                .ForMember(dest => dest.StudentMedicationId, opt => opt.MapFrom(src => src.StudentMedicationId))
        ...
        private static string GetStatusUsageDisplayName(StatusUsage status)
        {
            return status switch
            {
                StatusUsage.Used => "Đã sử dụng",
                StatusUsage.Skipped => "Bỏ qua",
                StatusUsage.Missed => "Bỏ lỡ",
                _ => status.ToString()
            };
        }
        ...
        private static string GetTimesOfDayDisplayName(string? timesOfDayJson)
        {
            ...
            try
            {
                ...
                if (timeOfDayInts != null && timeOfDayInts.Any())
                {
                    ...
                    return displayNames.Any() ? string.Join(", ", displayNames) : "Chưa thiết lập";
                }
